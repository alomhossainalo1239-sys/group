...
Requires:  BOT_TOKEN environment variable (see .env file next to this
           script -- loaded automatically if present).
...
if WEBHOOK_URL and not WEBHOOK_SECRET_TOKEN:
    logging.getLogger("mod_bot").warning(
        "WEBHOOK_URL is set but WEBHOOK_SECRET_TOKEN is empty -- the webhook will run "
        "WITHOUT request verification. Set WEBHOOK_SECRET_TOKEN to any random string "
        "to protect it, or ignore this if only BOT_TOKEN is needed for you."
    )
...
async def settings_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # NOTE: the security here is the live is_user_admin() check done below
    # in start_command -- whoever opens this link, we re-verify they are
    # actually an admin of THIS chat_id before letting them touch anything.
    deep_link = f"https://t.me/{bot_username}?start=cfg_{chat.id}"
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.effective_message.reply_text("That settings link looks invalid.")
        return
...
# =============================================================================
# This bot does its real work via Telegram polling, not HTTP -- it has no
# actual need for a port. But hosts like Render's "Web Service" type require
# every deploy to bind a port and will kill/restart the service otherwise
# (see the "No open ports detected" messages in the deploy logs). This tiny
# server exists ONLY to satisfy that check; it does not affect bot logic at
# all. If you deploy this as a Render "Background Worker" instead, none of
# this is needed (Background Workers don't require a port) -- but this way
# works fine too and needs no extra setup on Render's side.
class _HealthCheckHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        pass  # silence per-request logging; this endpoint is just noise


def _start_health_check_server():
    port = int(os.getenv("PORT", "10000"))  # Render sets PORT automatically
    server = HTTPServer(("0.0.0.0", port), _HealthCheckHandler)
    logger.info("Health-check HTTP server listening on port %s (for host port-binding checks only)", port)
...
def main():
    ...
    if WEBHOOK_URL:
        ...
        # Start the dummy health-check server in the background so Render
        # (or similar hosts) sees an open port, while the real bot logic
        # below still runs via polling as normal.
